...
function getDaysBetween(a,b) { const r=[],s=parseDate(a),e=parseDate(b),c=new Date(s); while(c<=e){r.push(toDateStrSimple(c));c.setDate(c.getDate()+1);} return r; }
  ...
  dates.forEach(date => {
    ...
    if (mobile) {
      ...
      SUBJECTS.forEach(subj => {
        ...
        // Read inp.value (live DOM) — row.plans only updates on blur/change
        const hasPlan = () => inp.value.trim().length > 0;
        const getMobileCls = () => 'pcard-tick' + (row.ticks[subj] ? ' ticked' : '') + (!hasPlan() && !row.ticks[subj] ? ' tick-disabled' : '');
        tick.className = getMobileCls();
      ...
      SUBJECTS.forEach(subj => {
        ...
        // Read inp.value (live DOM) — row.plans only updates on blur/change, so
        // typing without blurring would falsely disable the tick.
        const hasPlanD = () => inp.value.trim().length > 0;
        const getDeskCls = () => 'tick-btn' + (row.ticks[subj] ? ' ticked' : '') + (!hasPlanD() && !row.ticks[subj] ? ' tick-disabled' : '');
        btn.className = getDeskCls();
      ...
      abtn.addEventListener('click', () => {
        triggerHaptic(60);
        // Only consider subjects that have a saved plan
        const tickable = SUBJECTS.filter(s => (row.plans[s] || '').trim().length > 0);
        if (tickable.length === 0) { showToast('Add plans first before bulk-ticking 📝', 'info'); return; }
        const allT = tickable.every(s => row.ticks[s]);
        tickable.forEach(s => { row.ticks[s] = !allT; });
...
function renderTestTable() {
  ...
  if(mobile){
    ...
    sorted.forEach(function(t){
      ...
      // Safe cell builder — uses textContent to prevent XSS
      ...
      typeBadge.className='test-type-badge test-type-'+t.type;
      ...
      var badgeTd=document.createElement('td'); badgeTd.innerHTML=badgeHtml; tr.appendChild(badgeTd);

      var commentTd=document.createElement('td'); commentTd.className='comment-cell';
      commentTd.title=t.comment||'';
      commentTd.textContent=t.comment||'—';
      tr.appendChild(commentTd);

      var actTd=document.createElement('td'); actTd.style.cssText='display:flex;gap:6px;align-items:center;';
      var editBtn=document.createElement('span'); editBtn.className='test-edit-btn'; editBtn.title='Edit';
      editBtn.style.cssText='color:var(--accent);opacity:0;cursor:pointer;font-size:13px;'; editBtn.textContent='✎';
      var delBtn=document.createElement('span'); delBtn.className='test-del'; delBtn.title='Delete'; delBtn.textContent='✕';
      actTd.append(editBtn,delBtn); tr.appendChild(actTd);

      (function(id){
        delBtn.addEventListener('click',function(){data.tests=data.tests.filter(function(x){return x.id!==id;});saveData();renderTestTable();});
        editBtn.addEventListener('click',function(){openTestEditModal(id);});
      })(t.id);
      tbody.appendChild(tr);
    });
...
// ---- Subtab state ----
let reminderTab = 'active'; // 'active' | 'history'
...
// ---- Archive: move done reminders at/after midnight, prune > 30 days ----
function archiveDoneReminders() {
  ...
  if (toArchive.length) {
    toArchive.forEach(r => {
      data.reminderHistory.push(Object.assign({}, r, { archivedAt: toDateStrSimple(new Date()), archiveType: 'completed' }));
    });
...
// ---- Render Active Reminders tab ----
function renderReminders() {
  var activeTabEl  = document.getElementById('rem-tab-active');
  var historyTabEl = document.getElementById('rem-tab-history');
  var activeWrap   = document.getElementById('reminders-list-wrap');
  var historyWrap  = document.getElementById('reminder-history-wrap');
  var filterBar    = document.getElementById('reminder-filter-bar');
  ...
  var catFilter  = document.getElementById('rf-category').value;
  var statFilter = document.getElementById('rf-status').value;

  var reminders = [].concat(data.reminders || []).sort(function(a,b){ return a.datetime.localeCompare(b.datetime); });
  if (catFilter !== 'all') reminders = reminders.filter(function(r){ return r.category === catFilter; });
  if (statFilter === 'pending') reminders = reminders.filter(function(r){ return !r.done; });
  if (statFilter === 'done')    reminders = reminders.filter(function(r){ return r.done; });

  if (reminders.length === 0) {
    activeWrap.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text2);font-size:14px;">No reminders found. Add one to stay on track! 🚀</div>';
    return;
  }
  ...
  reminders.forEach(function(r) {
    ...
    var dateStr = dt.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' });
    var doneHint = r.done ? ' <span class="rcard-done-hint">✓ Done · moves to History at midnight</span>' : '';
    var subjHtml = r.subject ? '<span class="rcard-subject">📚 ' + (SUBJECT_LABELS[r.subject] || r.subject) + '</span>' : '';
    card.innerHTML =
...
function renderReminderHistory() {
  var wrap = document.getElementById('reminder-history-wrap');
  if (!wrap) return;
  wrap.innerHTML = '';
  var history = [].concat(data.reminderHistory || []).sort(function(a,b){ return (b.archivedAt||'').localeCompare(a.archivedAt||''); });
  if (history.length === 0) {
    wrap.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text2);font-size:14px;">No history yet. Completed and deleted reminders from the last 30 days appear here. 📋</div>';
    return;
  }
  history.forEach(function(h) {
    var card = document.createElement('div');
    card.className = 'rcard glass-card rcard-history';
    var dt = new Date(h.datetime);
    var timeStr = dt.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' });
    var dateStr = dt.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' });
    var isFuture = new Date(h.datetime) > new Date();
    var typeBadge = '<span class="rcard-history-badge ' + h.archiveType + '">' + (h.archiveType === 'deleted' ? 'Deleted' : 'Completed') + '</span>';
    var restoreBtn = isFuture ? '<button class="btn btn-surface rcard-restore-btn" onclick="restoreReminder(\'' + h.id + '\')">↩ Restore</button>' : '';
    var permDelBtn = '<button class="btn btn-danger rcard-perm-del-btn" onclick="permanentlyDeleteReminder(\'' + h.id + '\')">🗑</button>';
    var subjHtml = h.subject ? '<span class="rcard-subject">📚 ' + (SUBJECT_LABELS[h.subject] || h.subject) + '</span>' : '';
    card.innerHTML =
      '<div class="rcard-body">' +
...
function openReminderModal() {
  ...
  var subSelect = document.getElementById('rm-subject');
...
function updateReminderSubjectVisibility() {
  var isStudy = document.getElementById('rm-category').value === 'Study';
...
function saveReminder() {
  ...
  var datetime = document.getElementById('rm-datetime').value;
  var category = document.getElementById('rm-category').value;
  var subject  = category === 'Study' ? document.getElementById('rm-subject').value : '';
  ...
  var id = 'rem_' + Date.now();
  var reminder = { id: id, title: title, datetime: datetime, category: category, subject: subject, done: false, notified: false, doneAt: null };
...
function toggleReminderDone(id) {
  var r = data.reminders.find(function(rem){ return rem.id === id; });
  if (r) {
    r.done = !r.done;
...
// Soft-delete → history immediately
function deleteReminderToHistory(id) {
  ...
  data.reminders = data.reminders.filter(function(rem){ return rem.id !== id; });
  if (notificationTimers[id]) { clearTimeout(notificationTimers[id]); delete notificationTimers[id]; }
  var cutoff = new Date(); cutoff.setDate(cutoff.getDate() - 30);
  data.reminderHistory = data.reminderHistory.filter(function(h){ return (h.archivedAt||'') >= toDateStrSimple(cutoff); });
  saveData();
  renderReminders();
...
function restoreReminder(id) {
  var h = (data.reminderHistory || []).find(function(x){ return x.id === id; });
  if (!h) return;
  var restored = { id: h.id, title: h.title, datetime: h.datetime, category: h.category, subject: h.subject, done: false, notified: false, doneAt: null };
  data.reminders.push(restored);
  data.reminderHistory = data.reminderHistory.filter(function(x){ return x.id !== id; });
  saveData();
...
function showLoginScreen() {
  // Remove auth hint so login screen is never hidden again after session expiry
